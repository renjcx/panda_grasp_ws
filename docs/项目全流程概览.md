...
过程中解决了 3 个 MoveItPy 兼容性 bug（参数名不匹配、规划管线加载失败、仿真时间下轨迹执行失败，详见 [`MoveItPy_Bug修复总结.md`](MoveItPy_Bug修复总结.md)）和 1 个 Gazebo 资源路径问题，踩坑过程已沉淀到 [`Gazebo踩坑总结.md`](Gazebo踩坑总结.md)。
